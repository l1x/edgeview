...
impl MonthReport {
    ...
    pub fn from_day_caches(mut days: Vec<DayCache>, exact_visitors: Option<(u64, u64)>) -> Self {
        ...
        let daily: Vec<DailyTraffic> = days.iter().map(|d| DailyTraffic {
            date: d.date,
            hits: d.hits,
            visitors: d.visitors,
        }).collect();
        ...
        for day in &days {
            for p in &day.top_pages {
                let entry = page_rollup.entry((p.category.clone(), p.path.clone())).or_default();
                entry.0 += p.hits;
                entry.1 += p.visitors;
                entry.2 += p.bot_hits;
            }
        }
        let mut top_pages: Vec<PageHits> = page_rollup.into_iter()
            .map(|((category, path), (hits, visitors, bot_hits))| PageHits {
                path, hits, visitors, bot_hits, category,
            })
            .collect();
        ...
        for day in &days {
            for b in &day.bot_stats {
                let entry = bot_rollup.entry(b.bot_name.clone()).or_insert(CrawlerStats {
...
impl MonthSummary {
    pub fn from_month_report(report: &MonthReport, month: &str) -> Self {
        MonthSummary {
            ...
            daily: report.daily.iter().map(|d| DailyTraffic {
                date: d.date,
                hits: d.hits,
                visitors: d.visitors,
            }).collect(),
            top_pages: report.top_pages.clone(),
            bot_stats: report.bot_stats.iter().map(|b| CrawlerStats {
                bot_name: b.bot_name.clone(),
                hits: b.hits,
...
impl YearReport {
    pub fn from_month_data(year: &str, months: Vec<(String, MonthReport)>, exact_visitors: (u64, u64)) -> Self {
        ...
        let monthly: Vec<MonthlyTraffic> = months.iter().map(|(m, r)| MonthlyTraffic {
            month: m.clone(),
            hits: r.total_hits,
        ...
        let month_summaries: Vec<MonthSummary> = months.iter()
